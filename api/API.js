...
export class API {
    ...
    static async postRequest(endpoint, data) {
        const response = await axios.post(this.api_link + endpoint, { data }, { withCredentials: true });
        const result = response.data
        return result
    }
    static async putRequest(endpoint, data) {
        const response = await axios.put(this.api_link + endpoint, { data }, { withCredentials: true });
        const result = response.data;
        return result
    }
